...
_confirmMsg = format ["This airdrop will deduct $80,000 from your bank account<br/>"];
_confirmMsg = _confirmMsg + format ["<br/><t font='EtelkaMonospaceProBold'>1</t> x IFV-6a Cheetah AA"];
...
{	// Include vehicle store cooldown timer
	...
	if (_timeLeft > 0) then
	{
		...
		_para = createVehicle ["B_Parachute_02_F", _posplr, [], 0, ""];
			_spwnveh = "B_APC_Tracked_01_AA_F" createVehicle _posplr;
		...
		if (["A3W_playerSaving"] call isConfigOn) then
		{
			...
			_spwnveh setVariable ["R3F_LOG_disabled", true, true];	
			_spwnveh lock 2; // Spawn vehicles in locked	
			_getInOut =
		{
			...
			_spwnveh disableTIEquipment true;
		};		
/*
	...
	Function: extDB22_Database_async
	Author: Bryan "Tonic" Boardwine

	Description:
	Commits an asynchronous call to extDB22
	Gets result via extDB22  4:x + uses 5:x if message is Multi-Part
...
_key = "extDB22" callExtension (format ["%1:%2:", _mode, call A3W_extDB22_ID] + _queryStmt);

if(_mode == 1) exitWith
{
	_buffer = toArray (format ["DEBUG ----- extDB22 ASync: Complete Time:%1  Input String:", diag_tickTime - _tickTime] + _queryStmt);
...
_buffer = toArray (format ["DEBUG ----- extDB2 ASync: Complete Time:%1  Input String:", diag_tickTime - _tickTime] + _queryStmt);
while {count _buffer > 0} do
{
	diag_log toString (_buffer select [0, 1021]);
	_buffer deleteRange [0, 1021];
